...
### Overview
To accelerate training, we extract 1,280-dimensional embeddings from the raw audio files using Google's **Perch v1** model. This allows us to train custom classifier heads on fixed-size vectors instead of processing raw waveforms in every epoch.
...
- **Logic:**
    ...
    3. Passes the audio through the Perch v1 model.
    4. Extracts the `embedding` output.

### Artifacts Generated
These files are stored in `data/processed/` (ignored by git, must be generated locally):

1.  **`perch_v1_embeddings.npz`**: A compressed NumPy archive containing:
    - `embeddings`: A matrix of shape `(N, 1280)`.
    - `metadata_indices`: The original indices from `train.csv` corresponding to each embedding.
2.  **`train_with_embeddings.csv`**: A copy of the training metadata that only includes successfully processed files, with an additional `embedding_idx` column for easy lookup.
