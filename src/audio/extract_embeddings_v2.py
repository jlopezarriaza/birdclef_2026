...
# FORCE TF SETTINGS FOR CLOUD CPU
os.environ["TF_CPP_MIN_LOG_LEVEL"] = "3"
# Standard auto_jit for Cloud CPU
...
def load_model_v2():
    """Load Perch v2 using the official URL."""
...
def process_file_worker(filename, raw_dir):
    ...
    try:
        ...
        # Canonical signature call for Perch v2
        infer = model_instance.signatures['serving_default']
        # V2 SavedModel usually uses 'inputs'
        input_key = list(infer.structured_input_signature[1].keys())[0]
        outputs = infer(**{input_key: inputs})
        
        # Extract embedding (Output 1)
...
def main():
    ...
    print(f"Vertex AI Mode (V2): Using {args.workers} workers...")
