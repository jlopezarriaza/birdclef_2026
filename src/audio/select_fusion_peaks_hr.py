...
def main():
    ...
    if bucket:
        ...
        if not os.path.exists(registry_path):
            os.system(f"gsutil cp gs://{bucket}/processed/species_registry.json {registry_path}")
        if not os.path.exists(train_perch_csv):
            os.system(f"gsutil cp gs://{bucket}/processed/train_with_perch_v1.csv {train_perch_csv}")
        if not os.path.exists(model_path):
            os.system(f"gsutil cp gs://{bucket}/models/fusion_model_v1.keras {model_path}")
        if not os.path.exists(os.path.join(raw_dir, "train.csv")):
            os.system(f"gsutil cp gs://{bucket}/raw/train.csv {os.path.join(raw_dir, 'train.csv')}")
            
        # Download Audio (CRITICAL: this is big)
    ...
    if bucket:
        print(f"Uploading final results to gs://{bucket}/processed/train_v2_peaks.csv...")
        os.system(f"gsutil cp {output_path} gs://{bucket}/processed/train_v2_peaks.csv")
