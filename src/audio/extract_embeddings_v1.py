...
def process_file_worker(filename, raw_dir):
    ...
    try:
        ...
        # Check signature input keys
        input_key = list(infer.structured_input_signature[1].keys())[0]
        outputs = infer(**{input_key: inputs})
        
        # Perch v1 (v4) uses output_0 for labels and output_1 for embeddings
...
def main():
    ...
    # DOWNLOAD ONCE in the main process
    ...
    print(f"CPU Mode (V1): Using {args.workers} workers...")
    ...
    if valid_results:
        ...
        final_output_path = os.path.join(processed_dir, "perch_v1_embeddings.npz")
        final_csv_path = os.path.join(processed_dir, "train_with_perch_v1.csv")
        ...
        if args.gcs_bucket:
            try:
                ...
                bucket.blob("processed/perch_v1_embeddings.npz").upload_from_filename(final_output_path)
                bucket.blob("processed/train_with_perch_v1.csv").upload_from_filename(final_csv_path)
                print(f"Uploaded v1 results to gs://{args.gcs_bucket}")
            except Exception as e:
